...
import {currencyFormat, filterArray, filterArrayCitation, returnCitations, totalFines} from "./Stats";
    ...
    function eachRow(header, arr) {
        ...
        return (
            <tr>
                ...
                }}><h4 className={"alignCenter"}>{reports} ({((reports / totalReportsLength) * 100).toFixed(0)}%)</h4>
                </Link></td>
                <td><Link to={{
                    pathname: "/data",
                    state: {
                    ...
                    className={"alignCenter"}>{citations} ({((citations / totalCitations.length) * 100).toFixed(0)}%)</h4>
                </Link></td>
            </tr>
        )
    }
